...
def get_rir(
    ...
    *,
    ...
    )  # per-dimension inter-cell lengths in the lattice
    for order_i in range(1, order + 1):
        order_parity = order_i % 2
        lattice_coords[..., order_i, :, 0] = (
            lattice_coords[..., order_i - 1, :, 0] - lattice_d[..., 1 - order_parity, :]
        )
        lattice_coords[..., order_i, :, 1] = (
            lattice_coords[..., order_i - 1, :, 1] + lattice_d[..., order_parity, :]
        )
        lattice_ref[..., order_i, :, 0] = (
            -1.0
            * lattice_ref[..., order_i - 1, :, 0]
            * room_mat[..., :, 1 - order_parity]
        )
        lattice_ref[..., order_i, :, 1] = (
            -1.0 * lattice_ref[..., order_i - 1, :, 1] * room_mat[..., :, order_parity]
        )
    ...
    for l_i in range(order + 1):
        for l_j in range(order - l_i + 1):
            for l_k in range(order - l_i - l_j + 1):
                ...
                quadrant_mask = np.all(
                    np.logical_not((l_ijk[:, np.newaxis] == 0) & (quadrant_b != 0)),
                    axis=0,
                )
                for quad_i in range(8):
                    ...
                    if quadrant_mask[quad_i]:
                        ...
                        im_loc = torch.stack(
                            [
                                lattice_coords[..., l_i, 0, quad_bi[0]],
                                lattice_coords[..., l_j, 1, quad_bi[1]],
                                lattice_coords[..., l_k, 2, quad_bi[2]],
                            ],
                            dim=-1,
                        )
                        im_ref = (
                            lattice_ref[..., l_i, 0, quad_bi[0]]
                        ...
                        rir = rir + im_ref * atten * impulse_func(
                            timestamp - (delay + delay_offset)
                        )
    ...
    if render_tail:
        ...
        max_order_decay = (
            torch.log(torch.abs(max_order_amp) + torch.finfo(torch.float32).eps * 1000)
            / max_order_delay
        )
        tail_env = tail_func(
            (timestamp + tail_offset)[..., None],
            max_order_decay,
            max_order_delay * (order + 1.0) / order,
        )
        tail = torch.sum(
            0.5
...
class SmoothGradISM(autograd.Function):
    ...
    def forward(ctx, input, options):
        ...
        attenuation = (
            options["attenuation"] if ("attenuation" in options) else ATTENUATION
        )
        ...
        with torch.enable_grad():
            ...
            direct_delay = (
                torch.sqrt(
                    torch.sum(torch.square(src_loc - mic_loc), dim=-1, keepdim=True)
                )
                / mach
            )
            if normalize:
            ...
            # shape of (order, 3, 2) for order max reflections, 3 dimensions, and 2 directions (negative, positive); computed independently in each dim
            lattice_coords = torch.empty(
                batch_dims + (order + 1, 3, 2), requires_grad=False
            )
            lattice_coords[..., 0, :, :] = torch.unsqueeze(
                src_loc.clone(), dim=-1
            ).expand(src_loc.shape + (2,))
            lattice_ref = torch.empty(
                batch_dims + (order + 1, 3, 2), requires_grad=False
            )
            lattice_ref[..., 0, :, :] = torch.ones(batch_dims + (3, 2))
            ...
            for order_i in range(1, order + 1):
                order_parity = order_i % 2
                lattice_coords[..., order_i, :, 0] = (
                    lattice_coords[..., order_i - 1, :, 0]
                    - lattice_d[..., 1 - order_parity, :]
                )
                lattice_coords[..., order_i, :, 1] = (
                    lattice_coords[..., order_i - 1, :, 1]
                    + lattice_d[..., order_parity, :]
                )
                lattice_ref[..., order_i, :, 0] = (
                    -1.0
                    * lattice_ref[..., order_i - 1, :, 0]
                ...
                lattice_ref[..., order_i, :, 1] = (
                    -1.0
                    * lattice_ref[..., order_i - 1, :, 1]
                    * room_mat[..., :, order_parity]
                )
            # quadrant bits, for convenience in traversing lattice
            quadrant_b = np.array(
                [
            ...
            for l_i in range(order + 1):
                for l_j in range(order - l_i + 1):
                    for l_k in range(order - l_i - l_j + 1):
                        ...
                        quadrant_mask = np.all(
                            np.logical_not(
                                (l_ijk[:, np.newaxis] == 0) & (quadrant_b != 0)
                            ),
                            axis=0,
                        )
                        for quad_i in range(8):
                            ...
                            if quadrant_mask[quad_i]:
                                ...
                                # delay, atten = compute_path_t(im_loc, mic_loc, attenuation)
                                im_dist = torch.sqrt(
                                    torch.sum(torch.square(im_loc - mic_loc), dim=-1)
                                )
                                delay = im_dist / MACH
                                ...
                                rir_g = rir_g + im_ref * atten * impulse_func_g(
                                    timestamp - (delay + delay_offset)
                                )
            # render approximate tail
            if render_tail:
                ...
                max_order_decay = (
                    torch.log(
                        torch.abs(max_order_amp) + torch.finfo(torch.float32).eps * 1000
                    )
                    / max_order_delay
                )
                tail_env = tail_func(
