...
def rir_diff(x, y, win_len=512, win_stride=128):
    ...
    x_tf = torch.stft(
        x, n_fft=win_len, hop_length=win_stride, window=win, return_complex=True
    )
    y_tf = torch.stft(
        y, n_fft=win_len, hop_length=win_stride, window=win, return_complex=True
    )
    ...
    return torch.mean(torch.square(x_mag - y_mag)) + torch.mean(
        torch.abs(x_arg - y_arg)
    )
...
def c80(rir, srate=SRATE):
    ...
    return 10.0 * torch.log10(
        torch.sum(rir_sq[:idx_80ms]) / torch.sum(rir_sq[idx_80ms:])
    )
...
def reverb_time(rir, tail_cutoff=0.25, srate=SRATE, eps=EPS):
    ...
    decay_rate = idx_cutoff * torch.sum(timestamp * decay_log) - torch.sum(
        timestamp
    ) * torch.sum(decay_log)
    decay_rate = decay_rate / (
        idx_cutoff * torch.sum(torch.square(timestamp))
        - torch.square(torch.sum(timestamp))
    )
...
def mtf(rir, snr=None):
    ...
    transfer_f = torch.sum(
        torch.exp(-2.0j * torch.pi * f_mod * timestamp) * rir_sq, dim=2
    )
